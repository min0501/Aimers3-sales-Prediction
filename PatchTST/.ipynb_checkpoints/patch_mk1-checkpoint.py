...
train_data = train_data.drop(columns=["쇼핑몰"])
output_kuka = train_data.iloc[:, -CFG["PREDICT_SIZE"] :].to_numpy()
product_features = pd.read_csv("/root/workspace/data/features2.csv").drop(
    columns=["Unnamed: 0", "ID", "제품", "대분류", "중분류", "소분류", "브랜드"]
)
time_series_features = pd.read_csv("/root/workspace/data/total_dates_scaling.csv").drop(
...
val_size = int(data_len * 0.2)+1
print(data_len, train_size, val_size)
...
torch.save(infer_model.state_dict(), "./ckpt/patchtst_submit_0916_1.pth")
