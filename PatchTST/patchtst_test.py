...
product_features = pd.read_csv("../dataset/features_old.csv").drop( 
    columns=["Unnamed: 0", "ID", "제품", "대분류", "중분류", "소분류", "브랜드"]
) # encoding 문제로 기존에 사용하던 파일 첨부하였습니다. 소수점 읽어오는 것이 다릅니다.
...
infer_model.load_state_dict(torch.load("./ckpt/patchtst_submit_0824.pth"))
# infer_model.load_state_dict(torch.load("./ckpt/patchtst_submit_0824_ver1.pth"))
...
submit.iloc[:, 1:] = pred
submit.to_csv("../submit/patchtst_submit_0824.csv", index=False)
