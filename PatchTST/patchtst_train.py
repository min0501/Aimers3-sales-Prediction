...
product_features = pd.read_csv("../../data/features_old.csv").drop(
    columns=["Unnamed: 0","ID", "제품", "대분류", "중분류", "소분류", "브랜드"]
)
product_features = product_features[cond]
time_series_features = pd.read_csv("../../data/total_dates_scaling.csv").drop(
    columns=["Date"]
)
...
torch.save(infer_model.state_dict(), "./ckpt/patchtst_submit_0824.pth")
